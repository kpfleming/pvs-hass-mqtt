...
from hass_mqtt import SensorStateClass, UnitOfMeasurement
...
def test_default_state_class() -> None:
    ...
    b = GenericSensor(name="foo", value=0)
    assert b.state_class is None


def test_state_class() -> None:
    """Ensure that a provided state class is set."""
    b = GenericSensor(name="foo", value=0, state_class=SensorStateClass.total)
...
def test_default_unit_of_measurement() -> None:
    ...
    b = GenericSensor(name="foo", value=0)
    assert b.unit_of_measurement is None
