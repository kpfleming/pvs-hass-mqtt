...
def test_client_id(mock_client: MagicMock) -> None:
    ...
    mqtt = MQTT(client_id="baz", broker="", port=0, keep_alive=0, qos=0, discovery_prefix="")
    mqtt.connect()
    mock_client.assert_called_once()
    assert "client_id" in mock_client.call_args.kwargs
...
def test_logger_enabled(mock_client: MagicMock) -> None:
    ...
    MQTT(broker="", port=0, keep_alive=0, qos=0, discovery_prefix="").connect()
...
def test_username(mock_client: MagicMock) -> None:
    ...
    MQTT(
        broker="", port=0, keep_alive=0, qos=0, discovery_prefix="", username="foo", password="bar"
    ).connect()
...
def test_broker_port_keep_alive(mock_client: MagicMock) -> None:
    ...
    MQTT(broker="baz", port=42, keep_alive=39, qos=0, discovery_prefix="").connect()
